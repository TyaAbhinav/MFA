...
def file_call_microphone(username):
    try:
        file_name = username+".wav"
        #unique_id = str(shortuuid.uuid())
        #unique_filename = f"{file_name}_{unique_id}.wav"
        output_directory = f"registered_voice/{file_name[:-4]}/"
        os.makedirs(output_directory, exist_ok=True)
        for attempt in range(3):
            G1, output_directory = write_file_for_microphone(attempt, file_name, output_directory)
            with open(f"{output_directory}Voicerecognition.pkl","wb") as pkl3:
                pickle.dump(G1,pkl3)
        #print(f"Your Username is : {unique_filename[:-4]}")
    except:
